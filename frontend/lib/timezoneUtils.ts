...
export function getTimezoneOffset(dateStr: string, location: string): string {
    // 1. Default to Japan if no location provided or explicit Japan check passes
    if (!location || location.toLowerCase().includes('japan') || isJapanesePrefecture(location)) {
    ...
    const scored = candidates.map(c => {
        ...
        // Boost by ISO codes
        if (c.iso2 && locationLower.includes(c.iso2.toLowerCase())) score += 20;
        if (c.iso3 && locationLower.includes(c.iso3.toLowerCase())) score += 20;
...
function getOffsetForZone(dateStr: string, timeZone: string): string {
    try {

...
class Importer:
    ...
    def save_results(
        ...
        try:
            ...
            for source in successful_sources:
                pat = self.SOURCE_PATTERNS.get(source)
                if pat:
    ...
    def _upsert_events(self, events: List[Event]):
        ...
        for e in events:
            ...
            for field in [
                ...
                if event_dict.get(field) is not None and not isinstance(
                    event_dict[field], list
                ):
    ...
    def _delete_missing_events(self, source_urls: Dict[str, List[str]]):
        ...
        for source, found_urls in source_urls.items():
            ...
            if not pattern:
                print(
                    f"  [Warn] No URL pattern defined for source '{source}'. Skipping clean up."
                )
                continue

            found_set = set(found_urls)
            print(
                f"  [{source}] Checking for missing events... (Found {len(found_set)} in this run)"
            )
            ...
            try:
                ...
                if to_delete:
                    print(
                        f"    -> Found {len(to_delete)} events in DB that are missing from source. Deleting..."
                    )
                    chunk_size = 200
                    ...
                    for i in range(0, len(to_delete), chunk_size):
                        ...
                        try:
                            self.supabase.table("events").delete().in_(
                                "url", chunk
                            ).execute()
                        except Exception as e:

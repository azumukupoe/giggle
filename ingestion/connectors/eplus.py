...
class EplusConnector(BaseConnector):
    ...
    async def _get_all_ids_async(
        ...
        try:
            ...
            print(
                f"  [eplus] Genre {genre_code} has {total_count} items. Fetching all..."
            )
            ...
            print(
                f"  [eplus] Fetched {len(ids)} unique exclusion IDs for genre {genre_code} out of {total_count} total items reported."
            )
            return ids
    ...
    def _process_item_sync(
        self, item: dict, dt_now: datetime, excluded_ids: Set[Tuple[str, str]]
    ) -> Optional[Event]:
        try:
            k_code = item.get("kogyo_code")
            k_sub = item.get("kogyo_sub_code")
            ...
            if koenbi_term:
                ...
                if times:
                    date_obj = date_obj.replace(
                        hour=times[0].hour, minute=times[0].minute
                    )
            ...
            if uketsuke_list:
                for uketsuke in uketsuke_list:
                    t_name = uketsuke.get("uketsuke_name_pc") or uketsuke.get(
                        "uketsuke_name_mobile"
                    )
                    if t_name:
                ...
                for uketsuke in uketsuke_list:
                    ...
                    if p_raw:
                        ...
                        p_cleaned = p_raw.replace(
                            "※2枚以上ご購入の方はお申込み前に同行者登録が必要です。同行者登録されていない場合お申込み手続きには進めません。\n同行者登録につきましてはこちら\nhttps://eplus.jp/sf/guide/fellow-ep\nをご確認ください。\nチケットには申込者･同行者共、会員登録の氏名が印字されます。",
                            "",
                        )
    ...
    async def _get_events_async(
        self, query: str = None, max_pages: int = None
    ) -> List[Event]:
        all_events = []
        dt_now = datetime.now()

        async with httpx.AsyncClient(
            limits=self.limits, timeout=self.timeout
        ) as client:
            excl_tasks = [
            ...
            for start_index in range(
                1 + items_per_page, total_count + 1, items_per_page
            ):
